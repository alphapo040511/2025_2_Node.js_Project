...
(1,1,1), -- 1 번 플레이어에 검 1개
(2,3,5), -- 2 번 플레이어에 물약 5개
(3,2,1) -- 3 번 플레이어에 방패 1개 players
...
-- 1. 새로운 아이템 추가
INSERT INTO items(name, DESCRIPTION, VALUE) VALUES
('짱짱썐검', '최종 무기 아무튼 좋음', 99)

-- 2. 특정 플레이어의 인벤토리에 새 아이템 추가
INSERT INTO inventories (player_id, item_id, quantity) VALUES
(1,4,1) -- 1 번 플레이어에 검 1개

-- 3. 가장 가치 있는 아이템 찾기 (ORDER BY value DESC)
SELECT p.username, i.name, inv.quantity, i.value
FROM players p
JOIN inventories inv ON p.player_id = inv.player_id
JOIN items i ON inv.item_id = i.item_id
...
(1,1),									-- 1 번 플레이어에게 초보자 퀘스트 할당
(2,2)										-- 2 번 플레이어에게 용사의 검 퀘스트 할당
...
-- 1. 새로운 퀘스트 추가
INSERT INTO quests(title, DESCRIPTION, reward_exp, reward_item_id) VALUES
('중급자', '튜토리얼을 모두 완료하세요.', 1000, 4)

-- 1-1. 플레이에게 새로운 퀘스트 할당
INSERT INTO player_quests(player_id, quest_id) VALUES
(1,3)
...
SELECT * FROM quests
ORDER BY reward_exp DESC LIMIT 1

SELECT title, reward_exp
FROM quests 
ORDER BY reward_exp DESC LIMIT 1