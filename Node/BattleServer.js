...
class BattleServer{
    constructor(port){
        ...
        this.clients = new Set();                       // 중복이 업는 자료 구조 (Javascripts)
        this.players = new Map();                       // 데이터와 키를 같이 저장할 수 있는 자료구조 (JavaScripts)
        this.waitingPlayers = [];                       // 매칭 대기중인 플레이어들
    ...
    processBattleAction(battle, attackerId, action)
    {
        ...
        {
            case 'attack' :
                damage = Math.floor(Math.random() * 15) + 10;               // 10 ~ 24의 데미지
            ...
            case 'skill' :
                damage = Math.floor(Math.random() * 25) + 20;               // 20 ~ 44의 데미지
        ...
        const actionResult = {
            ...
            player1Hp : this.players.get(player1Id).hp,
            player2Hp : this.players.get(player2Id).hp
        };
        ...
        if(player && player.inBattle)
        {
            ...
            if(battle)
            {
                ...
                this.sendToPlayer(opponetId, {
                    ...
                    message : '상대방이 연결을 종료했습니다. 당신이 승리 했습니다.'
                });

                // 상대방 상태 초기화
                const opponet = this.players.get(opponetId);
                if(opponet)
                {
                    opponet.inBattle = false;
                    opponet.battleId = null;
                }
