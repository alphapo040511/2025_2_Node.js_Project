...
// 사용자 데이터 및 리프래시 토큰 저장소 (실제는 데이터베이스에서 진행)
const users = [];
const refreshTokens = {};
...
app.post('/register', async(req, res) => {
    ...
    users.push({username, password: hashedPassword});                   // hash 값으로 비밀번호 변경
...
app.post('/login', async(req, res) => {
    ...
    {
        return req.status(400).json({error : '잘못된 사용자명 또는 비밀번호 입니다.'});
    }
