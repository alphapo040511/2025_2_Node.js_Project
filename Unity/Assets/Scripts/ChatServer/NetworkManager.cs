...
public class NetworkManager : MonoBehaviour
{
    ...
    [SerializeField] private GameObject remotePlayerPrefab;    // 다른 플레이어 Prefabs
    ...
    private async void ConnectToServer()
    {
        ...
        UpdateStatusText("연결중...", Color.yellow);
    ...
    private void UpdateRemotePlayer(string playrId, Vector3Data position, Vector3Data rotation)
    {
        if(!remotePlayers.ContainsKey(playrId))         // 플레이어가 없으면 생성
        {
            CreateRemotePlayer(playrId, position, rotation);
            return;
        }

        GameObject player = remotePlayers[playrId];
    ...
    private void CreateRemotePlayer(string playerId, Vector3Data position, Vector3Data rotation)
    {
        ...
        GameObject player = Instantiate(remotePlayerPrefab, pos, Quaternion.Euler(rot));
